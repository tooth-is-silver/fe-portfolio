...
- 이슈 레이블: `bug`, `enhancement`, `question`, `refactor`

### PR 리뷰 → 이슈 → 리팩토링 워크플로

PR 생성 후 반드시 아래 순서로 진행:

```
1. PR 코드 리뷰 (자체 리뷰)
   - 변경된 파일 전체 읽기
   - 코딩 컨벤션 위반, 중복 코드, 개선 가능한 부분 파악

2. 리뷰 결과 → 이슈 등록
   - 즉시 수정 필요 (버그): label=bug → 현재 PR에서 해결
   - 개선 권장 (코드 품질): label=refactor → 별도 이슈로 등록
   - 나중에 할 것 (기능): label=enhancement → 백로그 이슈로 등록

3. 현재 PR에서 처리할 이슈 → 수정 커밋 추가
   - 커밋 메시지: refactor: ... (Closes #이슈번호)

4. 다음 PR에서 처리할 이슈 → 이슈만 등록하고 머지
```

**자체 리뷰 체크리스트:**
- [ ] 모바일 퍼스트 미디어 쿼리 (min-width 기준)
- [ ] 순수 함수 분리 가능한 로직이 컴포넌트 안에 남아 있지 않은지
- [ ] CSS 변수 사용 (하드코딩 컬러 없는지)
- [ ] 터치 타겟 44px 이상
- [ ] TypeScript 타입 명시 (any 없는지)
